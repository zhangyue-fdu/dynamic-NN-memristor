...
class pointnet2_cls_ssg(nn.Module):
    def __init__(self, in_channels, nclasses):
        ...
        # self.pt_sa8 = PointNet_SA_Module_ternary(M=128, radius=0.4, K=64, in_channels=259, mlp=[128, 128, 256], group_all=False)
        # self.pt_sa9 = PointNet_SA_Module_ternary(M=128, radius=0.4, K=64, in_channels=259, mlp=[128, 128, 256], group_all=False)
        # self.pt_sa10 = PointNet_SA_Module_ternary(M=128, radius=0.4, K=64, in_channels=259, mlp=[128, 128, 256], group_all=False)
        # self.pt_sa11 = PointNet_SA_Module_ternary(M=128, radius=0.4, K=64, in_channels=259, mlp=[128, 128, 256], group_all=False)
        self.pt_sa12 = PointNet_SA_Module_ternary(M=None, radius=None, K=None, in_channels=259, mlp=[256, 512, 1024], group_all=True)
    ...
    def forward(self, xyz, points):
        ...
        # new_xyz, new_points = self.pt_sa9(new_xyz, new_points)
        # new_xyz, new_points = self.pt_sa10(new_xyz, new_points)
        # new_xyz, new_points = self.pt_sa11(new_xyz, new_points)
        new_xyz, new_points = self.pt_sa12(new_xyz, new_points)
        net = new_points.view(batchsize, -1)
...
model.to(device)
# load_state_dict = torch.load('pointnet2_ternary.pth')
...
for i in range(epoch):  # 开始迭代
    ...
    if test_accuracy / test_num > 0.83:
        torch.save(model.state_dict(), "pointnet2_ternary_0.83_layer12.pth")
        print('save')
        test_loss_all.append(test_loss / test_num)
        test_accur_all.append(test_accuracy.double().item() / test_num)

torch.save(model.state_dict(), "pointnet2_ternary_layer12.pth")
print("模型已保存")