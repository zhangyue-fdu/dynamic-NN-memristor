...
class pointnet2_cls_ssg(nn.Module):
    def __init__(self, in_channels, nclasses):
        ...
        self.pt_sa71 = PointNet_SA_Module(M=128, radius=0.4, K=64, in_channels=259, mlp=[256, 512, 1024], group_all=False)
    ...
    def forward(self, xyz, points):
        ...
        confidence_threshold = [0.03, 0.03, 0.03, 0.03, 0.03, 0.01, 0.1, 0.005]
...
# 测试所保存的模型
m_state_dict = torch.load('pointnet2_raw_1_0.88.pth')
model.load_state_dict(m_state_dict)

semantic_center = torch.load('pointnet2_semantic_center_0.88.pth')
