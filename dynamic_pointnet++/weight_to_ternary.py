...
model.to(device)
load_state_dict = torch.load('pointnet2_ternary_0.83.pth')
model.load_state_dict(load_state_dict)

for name,parameters in model.named_parameters():
    if 'Conv' in name:
        print(name)
        load_state_dict[name] = load_state_dict[name] + gen_noise(load_state_dict[name], 0.10)
        # print(m_state_dict[name])
        # print(gen_noise(m_state_dict[name], 0.1))
        torch.save(load_state_dict,'pointnet2_ternary_noise10.pth')
